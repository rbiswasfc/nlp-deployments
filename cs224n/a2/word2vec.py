...
def naiveSoftmaxLossAndGradient(centerWordVec, outsideWordIdx, outsideVectors, dataset):
    ...
    p_i = softmax(np.dot(outsideVectors, centerWordVec.reshape(-1, 1)).T)
    loss = -np.log(p_i[0, outsideWordIdx])
    gradCenterVec = -u_o + np.dot(p_i, outsideVectors).squeeze()
    p_i[0, outsideWordIdx] -= 1
    gradOutsideVecs = np.outer(p_i.squeeze(), centerWordVec)
...
def negSamplingLossAndGradient(
    ...
    prob_indices = np.hstack(
        [sigmoid(np.dot(u_o, centerWordVec)), sigmoid(np.dot(U_k, centerWordVec))]
    )

    p = np.zeros(shape=(1, vocab_size))
    for i, idx in enumerate(indices):
        p[0, idx] += prob_indices[i]
    p[0, outsideWordIdx] -= 1

    # LOSS
    loss = (
        -np.log(sigmoid(np.dot(u_o, centerWordVec)))
        - np.log(sigmoid(-np.dot(U_k, centerWordVec))).sum()
    )
    gradCenterVec = np.dot(p, outsideVectors).squeeze()
    gradOutsideVecs = np.outer(p.squeeze(), centerWordVec)
...
def skipgram(
    ...
    for outsideWord in outsideWords:
        ...
        this_loss, this_gradCenterVec, this_gradOutsideVecs = word2vecLossAndGradient(
            centerWordVec, outsideWordIdx, outsideVectors, dataset
        )
        loss += this_loss
        gradCenterVecs[word2Ind[currentCenterWord], :] += this_gradCenterVec
        gradOutsideVectors += this_gradOutsideVecs
